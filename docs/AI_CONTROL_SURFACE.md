...
`inspect` returns focused JSON for `components`, `drill-actions`, `data-requirements`, or `react-backlog`, so agents can query the spec before deciding what to generate or validate.
